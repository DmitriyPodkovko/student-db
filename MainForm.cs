    ...
    public partial class MainForm : Form
    {
        ...
        private void dgvMain_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                ...
                {
                    this.Cursor = Cursors.WaitCursor;
                    if ((dgvMain.RowCount > 1) & (Rowcur > 0))
                    {
                        dgvMain.Rows[Rowcur].Selected = true;
                        SNMain = dgvMain.Rows[Rowcur].Cells["SN"].Value.ToString();
                        Rowcur = 0;
                    }
                    else SNMain = dgvMain.Rows[e.RowIndex].Cells["SN"].Value.ToString(); 
                    RefreshSchl();
                    RefreshSts();
                    RefreshPay();
                    RefreshSub();
                        ...
                        string newRegSchl, string newSchl, string newEdu, string newSts, string newRprt)
        {
            try
            {
                ...
                string SNmax = cmd.ExecuteScalar().ToString();
                cmd.Connection.Close();

                cmd.Connection.Open();
                cmd.CommandText = string.Format(strSqlInsAddres, SNmax, newRegAddr, newAddr);
                cmd.ExecuteReader();
                cmd.Connection.Close();

                cmd.Connection.Open();
                cmd.CommandText = string.Format(strSqlInsSchool, SNmax, newRegSchl, newSchl);
                cmd.ExecuteReader();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                AddForm fmAdd = new AddForm();
                Rowcur = dgvMain.CurrentRow.Index;
